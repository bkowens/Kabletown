...
import (
	...
	"github.com/bowens/kabletown/user-service/internal/handlers"
...
func main() {
	...
	if err := db.RunMigrations(sqlxDB); err != nil {
		log.Fatalf("user-service: migrations failed: %v", err)
	}
	...
	go func() {
		log.Printf("user-service: listening on %s (server-id=%s)", addr, serverID)
	...
	case sig := <-quit:
		log.Printf("user-service: received signal %v — shutting down", sig)
	}
	...
	if err := srv.Shutdown(ctx); err != nil {
		log.Fatalf("user-service: graceful shutdown failed: %v", err)
	}
